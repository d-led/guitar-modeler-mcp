...
type Request struct {
	...
	// AmpColour, CabColour, Amp2Colour and Cab2Colour override the slot colour
	// tag of the amp/cab modules; empty uses the factory-conventional colour
	// (amp Yellow, cab Green).
	AmpColour  string  `json:"amp_colour,omitempty"`
	CabColour  string  `json:"cab_colour,omitempty"`
	Amp2Colour string  `json:"amp2_colour,omitempty"`
	Cab2Colour string  `json:"cab2_colour,omitempty"`
...
func (d *Designer) Design(req Request) (*Result, error) {
	...
	colours, err := d.slotColoursFor(req)
	if err != nil {
		return nil, err
	}
	if err := d.applyRouting(&spec, req, ampModel, cabModel, micModel, skipCab, pre, post, last, pinned, colours, &notes); err != nil {
		return nil, err
	}
...
func (d *Designer) applyRouting(spec *rig.Spec, req Request, ampModel, cabModel, micModel string, skipCab bool, pre, post, last []rig.Block, pinned map[int]rig.Block, colours slotColours, notes *[]string) error {
	switch {
	case req.Routing == rig.RoutingSPS && req.Amp2 == "":
		// Shared amp: the amp+cab feed two parallel effect paths.
		spec.Prefix = append(pre, d.ampBlock(ampModel, colours.amp, req.AmpParams), d.cabBlock(cabModel, micModel, colours.cab, req.CabParams))
	...
	case req.Routing == rig.RoutingSPS:
		...
		pathA, pathB, note2, cab2Model, err := d.dualAmpPaths(req, ampModel, cabModel, micModel, colours)
		if err != nil {
			return err
		}
		spec.Prefix = pre
	...
	default:
		...
		blocks = append(blocks, d.ampBlock(ampModel, colours.amp, req.AmpParams))
		if !skipCab {
			blocks = append(blocks, d.cabBlock(cabModel, micModel, colours.cab, req.CabParams))
		}
...
func (d *Designer) dualAmpPaths(req Request, ampModel, cabModel, micModel string, colours slotColours) (pathA, pathB []rig.Block, note, cab2Model string, err error) {
	...
	pathA = []rig.Block{d.ampBlock(ampModel, colours.amp, req.AmpParams), d.cabBlock(cabModel, micModel, colours.cab, req.CabParams)}
	pathB = []rig.Block{d.ampBlock(amp2Model, colours.amp2, nil), d.cabBlock(cab2Model, mic2Model, colours.cab2, nil)}
...
func (d *Designer) classifyFX(fx []FXBlock) (pre, post, last []rig.Block, pinned map[int]rig.Block, err error) {
	...
	for _, f := range fx {
		...
		if err != nil {
			return nil, nil, nil, nil, fmt.Errorf("effect %q: %w", def.Name, err)
		}
		block := rig.Block{Type: def.Name, Enabled: f.Enabled, Params: f.Params, Colour: colour}
		if block.Params == nil {
...
func (d *Designer) fxBlocks(fx []FXBlock) ([]rig.Block, error) {
	...
	for _, f := range fx {
		...
		colour, err := d.colourFor(def.Name, f.Colour)
		if err != nil {
			return nil, fmt.Errorf("effect %q: %w", def.Name, err)
		}
		blocks = append(blocks, rig.Block{Type: def.Name, Enabled: f.Enabled, Params: f.Params, Colour: colour})
	}
	return blocks, nil
}

func (d *Designer) ampBlock(model, colour string, params map[string]any) rig.Block {
	...
	p["Type"] = model
	p["On"] = true
	return rig.Block{Type: "Amp", Enabled: true, Params: p, Colour: colour}
}

func (d *Designer) cabBlock(cab, mic, colour string, params map[string]any) rig.Block {
	...
	p["MicType"] = mic
	p["On"] = true
	return rig.Block{Type: "Cab", Enabled: true, Params: p, Colour: colour}
}

// slotColours holds the resolved slot-colour tags of the amp and cab modules,
// so the parallel amp paths and the serial chain all agree on them.
type slotColours struct {
	amp, cab, amp2, cab2 string
}

// slotColoursFor resolves the four amp/cab slot colours: a caller override
// wins (and must be a device palette colour), otherwise the module keeps its
// factory-conventional colour.
func (d *Designer) slotColoursFor(req Request) (slotColours, error) {
	var cols slotColours
	var err error
	if cols.amp, err = d.colourFor("Amp", req.AmpColour); err != nil {
		return cols, err
	}
	if cols.cab, err = d.colourFor("Cab", req.CabColour); err != nil {
		return cols, err
	}
	if cols.amp2, err = d.colourFor("Amp", req.Amp2Colour); err != nil {
		return cols, err
	}
	if cols.cab2, err = d.colourFor("Cab", req.Cab2Colour); err != nil {
		return cols, err
	}
	return cols, nil
}

// colourFor resolves one module's slot colour: the caller's override wins,
// otherwise the factory-conventional colour for the module type. An override
// must be one of the device palette colours.
...
// module type. It mirrors the dominant colour per effect category observed
// across the device's own presets (drives and EQs yellow, compressors red,
// delays green, reverbs blue, wahs orange, …), so a fresh rig reads like a
// stock one instead of every slot defaulting to green.
func (d *Designer) defaultColour(moduleType string) string {
	...
	case "modulation":
		switch def.Family {
		case "flanger", "phaser", "rotary":
			return "Orange"
		case "doubler":
			return "Red"
		case "octave", "harmonizer":
			return "Blue"
		case "filter":
			return "Yellow"
		}
