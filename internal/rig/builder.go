...
func (b *Builder) buildBlockNode(block Block) (*Node, error) {
	...
	if block.Colour != "" && !catalog.ColourValid(block.Colour) {
		return nil, fmt.Errorf("module %q: invalid colour %q (want one of: %s)", block.Type, block.Colour, catalog.ColourList())
	}
	...
	default:
		fxNode, err := buildFXNode(block.Type, block.Enabled, block.Params)
		if err != nil {
			return nil, err
		}
		node = fxNode
